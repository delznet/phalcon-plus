...
class Dir
{
     ...
     */
    public static function make(string $dir, int $mode = 0755):bool
    {
        //如果存在相同的文件名，则抛出异常
        if (is_file($dir)) {
     ...
     * @throws \InvalidArgumentException
     */
    public static function read(string $dir, string $includeRegEx = null, string $excludeRegEx = null):array
    {
        $files = [];
        if (!is_dir($dir)) {
     ...
     * @return bool
     */
    public static function delete(string $dir, bool $isIncludeSelf = false):bool
    {
        if (!is_dir($dir)) {
            return false;
        }
